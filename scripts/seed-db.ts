#!/usr/bin/env node
// Seeds MongoDB. Prefers migrating from the legacy SQLite database
// (data/portfolio.db) so live edits survive; falls back to
// data/portfolio.json + content/posts/*.mdx for a fresh install.
import fs from "fs";
import path from "path";
import matter from "gray-matter";
import mongoose from "mongoose";
import { Post, Portfolio } from "../src/lib/db/schema";
...
async function migrateFromSqlite(): Promise<boolean> {
  ...
  const portfolioRow = sqlite.prepare("SELECT data FROM portfolio WHERE id = 1").get() as
    ...
    | undefined;
  if (portfolioRow) {
    await Portfolio.findOneAndUpdate({}, { data: JSON.parse(portfolioRow.data) }, { upsert: true });
    console.log("✓ Portfolio migrated from SQLite");
  }
...
async function seedFromFiles() {
  if (fs.existsSync(JSON_PATH)) {
    ...
    await Portfolio.findOneAndUpdate({}, { data: raw }, { upsert: true });
...
async function main() {
  ...
  if (!migrated) {
    console.log("No SQLite database found — seeding from files");
    await seedFromFiles();
  }
